...
/* return 3 : HDMI CSI OK
 * return 2 : HDMI OK ; CSI not
 * return 1 : HDMI nor OK;
 * return 0 : VI not init;
 */
uint8_t get_vi_state()
{
	char VI_State[10]={0};
	char cmd[100] = "cat /proc/cvitek/vi_dbg | grep -A 1 VIDevFPS | awk '{print $3}'";
	uint8_t FPS[2];
	FILE* fp = popen( cmd, "r" );
...
uint8_t lt6911_get_hdmi_res()
{
    ...
    if(kvmv_cfg.hdmi_version == 0){
        ...
        if (Vactive != 0 && Hactive != 0){
            ...
            // system("echo 0 > /kvmapp/kvm/height");
            // system("echo 0 > /kvmapp/kvm/width");
            return 0;
        }
...
void* thread_hdmi_irq(void * arg)
{
    ...
    while(true)
    {
        // debug("[hdmi]thread - while\r\n");
        // debug("[hdmi]-\n");
        if(kvmv_cfg.try_exit_thread == 1)
            break;
		fp = fopen("/proc/lt_int", "r+");
		if(fp != NULL){
            // fseek(fp, 0, SEEK_END);
            // file_size = ftell(fp); 
            // fseek(fp, 0, SEEK_SET);
...
void kvmv_init(uint8_t _debug_info_en)
{
    ...
    if(kvmv_cfg.thread_is_running == 1){
        ...
        if (0 != pthread_create(&thread, NULL, thread_hdmi_irq, NULL)) {
...
int kvmv_read_img(uint16_t _width, uint16_t _height, uint8_t _type, uint16_t _qlty, uint8_t** _pp_kvm_data, uint32_t* _p_kvmv_data_size)
{
    ...
    do {
        ...
        // debug("[kvmv]kvmv_read_img - 6\r\n");
    } while (check_kvmv(try_num++));
    // debug("[kvmv]return: %d \r\n", (int)(time::time_ms() - start_time));
    *_pp_kvm_data = NULL;
        // debug("[kvmv]kvmv_read_img - end\r\n");
